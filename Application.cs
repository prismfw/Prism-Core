...
namespace Prism
{
    ...
    public abstract class Application : FrameworkObject
    {
        ...
        [DebuggerBrowsable(DebuggerBrowsableState.Never)]
#endif
        private static readonly Dictionary<NavigationAttribute, TypeLoader> controllerMap = new Dictionary<NavigationAttribute, TypeLoader>();
#if !DEBUG
        [DebuggerBrowsable(DebuggerBrowsableState.Never)]
#endif
        ...
        public static void Navigate(string uri, NavigationOptions options, IView fromView)
        {
            ...
            var mapEntry = controllerMap.FirstOrDefault(e => e.Key.UriPattern != null && IsMatch(uriParts, e.Key.UriPattern.Split('/')));
            var loader = mapEntry.Value;
            if (loader == null)
            {
        ...
        /// <summary>
        /// Registers the specified type and implementation for the <see cref="Resolve"/> methods.
        ...
        public static void Register(Type registerType, Type implementationType)
        {
            Register(registerType, implementationType, null, null);
        }
        ...
        public static void Register(Type registerType, Type implementationType, string name)
        {
            Register(registerType, implementationType, name, null);
        }
        ...
        public static void Register(Type registerType, Type implementationType, string name, string initializeMethod)
        {
            typeMap[registerType, name] = new TypeLoader(implementationType, false, initializeMethod);
        }

        /// <summary>
        /// Registers the specified type and implementation for the <see cref="Resolve"/> methods.  The implementation type will be
        ...
        /// <param name="registerType">The type to register.</param>
        /// <param name="implementationType">The type to instantiate when resolving the registered type.</param>
        public static void RegisterSingleton(Type registerType, Type implementationType)
        {
            RegisterSingleton(registerType, implementationType, null, null);
        }
        ...
        /// <param name="implementationType">The type to instantiate when resolving the registered type.</param>
        /// <param name="name">An optional unique identifier for the implementation type.</param>
        public static void RegisterSingleton(Type registerType, Type implementationType, string name)
        {
            RegisterSingleton(registerType, implementationType, name, null);
        }
        ...
        /// <param name="name">An optional unique identifier for the implementation type.</param>
        /// <param name="initializeMethod">An optional name of a static method on the implementation type to use in place of a constructor for initializing the object.</param>
        public static void RegisterSingleton(Type registerType, Type implementationType, string name, string initializeMethod)
        {
            typeMap[registerType, name] = new TypeLoader(implementationType, true, initializeMethod);
        }

        /// <summary>
        /// Registers the specified type and object instance for the <see cref="Resolve"/> methods.
        ...
        public static void RegisterSingleton(Type registerType, object instance)
        {
            RegisterSingleton(registerType, instance, null);
        }
        ...
        public static void RegisterSingleton(Type registerType, object instance, string name)
        {
            typeMap[registerType, name] = new TypeLoader(instance);
        }
        ...
        /// <param name="parameters">An array of parameters to pass to the constructor of the affiliated implementation during initialization.</param>
        ...
        {
            T retval;
            typeMap.TryResolve(typeof(T), name, parameters, out retval);
            return retval;
        }
        ...
        /// <param name="name">An optional unique identifier for the affiliated implementation.</param>
        /// <param name="parameters">An array of parameters to pass to the constructor of the affiliated implementation during initialization.</param>
        public static object Resolve(Type resolveType, string name, params object[] parameters)
        {
            object retval;
            typeMap.TryResolve(resolveType, name, parameters, out retval);
            return retval;
        }
        #endregion
        ...
        internal static void Initialize(Application appInstance, Assembly[] appAssemblies)
        {
            if (appAssemblies != null)
            {
                ...
                foreach (var type in appAssemblies.SelectMany(a => a.ExportedTypes))
                {
                    ...
                    {
                        typeMap[att.RegisterType, att.Name] = new TypeLoader(type, att.IsSingleton, att.InitializeMethod);
                    }
                    ...
                    if (navAtts.Any())
                    {
                        ...
                        {
                            controllerMap[att] = new TypeLoader(type, att.IsSingleton);
                        }
                        continue;
                    }
                    ...
                    foreach (var att in viewAtts)
                    {
                        ...
                        viewMap[att] = new TypeLoader(type, att.IsSingleton);
                    }
                }
                ...
                Logger.Info(CultureInfo.CurrentCulture, typeMap.Count == 1 ? Resources.Strings.RegisteredTypesToIoCContainerSingular : Resources.Strings.RegisteredTypesToIoCContainerPlural, typeMap.Count);
                Logger.Info(CultureInfo.CurrentCulture, controllerMap.Count == 1 ? Resources.Strings.FoundControllersWithNavAttributesSingular : Resources.Strings.FoundControllersWithNavAttributesPlural, controllerMap.Count);
                Logger.Info(CultureInfo.CurrentCulture, viewMap.Count == 1 ? Resources.Strings.FoundViewsWithPerspectivesSingular : Resources.Strings.FoundViewsWithPerspectivesPlural, viewMap.Count);
            }

            if (appInstance.nativeObject == null)
            {
                appInstance.nativeObject = Resolve<INativeApplication>();
                if (appInstance.nativeObject == null)
                {
